...
class aspect_dataset(Dataset):
    def __init__(self, file, args) -> None:
        ...
        self.pretrain_path = os.path.join(args.weight_dir, "pretrain")
        self.tokenizer = AutoTokenizer.from_pretrained(os.path.join(self.pretrain_path, "deberta"), add_prefix_space=True)
        self.image_model = 'vit-base-patch16-224-in21k'
        self.image_model_path = os.path.join(self.pretrain_path, self.image_model)
    ...
    def tokenize_data(self, sentence_l, image_l, label_l, pair_l, senti_l, allabel_l):
        ...
        for image_path in tqdm(image_l, desc="image"):
            img_path = os.path.join(self.args.path_image, image_path)
    ...
    def process_data(self, refresh_data=True):
        ...
        inputs_dir = os.path.join(self.args.dataset_dir, file_name)
...
class aspect_method():
    def __init__(self, args) -> None:
        ...
        self.dev_data = aspect_dataset(self.args.data_file['dev'], self.args)
        # train_pairs = train_data.pairs
        # dev_pairs = dev_data.pairs
        

    def model_select(self, text_model_name='deberta'):
        if text_model_name == 'bert':
            model_path1 = os.path.join(self.pretrain_path, 'bert')
            text_pretrained_dict = BertForTokenClassification.from_pretrained(model_path1).state_dict()
        elif text_model_name == 'roberta': 
            model_path1 = os.path.join(self.pretrain_path, 'roberta')
        ...
        image_model_path = os.path.join(self.pretrain_path, self.image_model)
        image_config = AutoConfig.from_pretrained(image_model_path)
        image_pretrained_dict = ViTForImageClassification.from_pretrained(image_model_path).state_dict()
    ...
    def train(self):
        train_dataloader = DataLoader(self.train_data, batch_size=self.args.aspect_batch_size)
        dev_dataloader = DataLoader(self.dev_data, batch_size=self.args.aspect_batch_size)

        # Load Model
        text_config, image_config, text_pretrained_dict, image_pretrained_dict = self.model_select("deberta")
        model = ASPModel(text_config, image_config, text_num_labels=3, alpha=self.args.alpha, beta=self.args.beta)  # 3=BIO
        ...
        optimizer = AdamW(optimizer_grouped_parameters, lr=self.args.aspect_lr, eps=self.args.adam_epsilon, no_deprecation_warning=True)
        ...
        for epoch in range(int(self.args.aspect_epochs)):
            ...
            if results["aspect_f1"] >= best_result["aspect_f1"]:
                ...
                # save model 
                print(results)
                torch.save(model, os.path.join(self.args.dataset_dir, "predict.pt"))
    ...
    def predict(self, type="train"):
        ...
        dataloader = DataLoader(data, batch_size=self.args.aspect_batch_size)
        checkpoint_path = os.path.join(self.args.dataset_dir, "predict.pt")

        if os.path.exists(checkpoint_path):
            print(checkpoint_path, "exists, loading")
            text_config, image_config, text_pretrained_dict, image_pretrained_dict = self.model_select("deberta")
            apsect_predict_model = ASPModel(text_config, image_config, text_num_labels=3, alpha=self.args.alpha, beta=self.args.beta)
